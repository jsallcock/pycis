...
def render_schematic(fpath_config, fpath_out, show_axes=True, show_cut_angle=True, show_label_details=True,
                     pol_state=None, show_label_pol_state=True, border=IMG_BORDER):
    """
    ...
    if show_axes:
        ...
        add_text_3d('x', [RADIUS, 0.28 * RADIUS, -edge_distance], **kwargs)
        add_text_3d('y', [0, 1.33 * RADIUS, -edge_distance], **kwargs)
        add_text_3d('z', [0, 0.3 * RADIUS, width_total + edge_distance * 0.85], **kwargs)
    # ------------------------------------------------------------------------------------------------------------------
    # INCIDENT LIGHT
    ...
    if mode == 'scan_polarisation':
        ...
        for ii, fp_config in enumerate(fpath_config):
            ...
            if ii == 0:
                ...
                show_label_pol_state = False
            render_schematic(fp_config, fp_out_schem, show_axes=True, show_cut_angle=True, show_label_details=False,
                             pol_state=pol_state[ii], show_label_pol_state=show_label_pol_state, border=0)
...
def render_igram(fpath_config, fpath_out, pol_state, ):
    ...
    spec = get_spectrum_delta_pol(465e-9, 5e3, pol_state['p'], pol_state['psi'], pol_state['xi'])
    ...
    dim = igram.shape
    igram_show = igram[
                 int(dim[0] / 2) - int(DIM_IGRAM_PIXELS / 2):int(dim[0] / 2) + int(DIM_IGRAM_PIXELS / 2),
                 int(dim[1] / 2) - int(DIM_IGRAM_PIXELS / 2):int(dim[1] / 2) + int(DIM_IGRAM_PIXELS / 2),
                 ]
    vmax = float(1.05 * igram_show.max())
    igram_show.plot(x='x', y='y', ax=axes[0], add_colorbar=False, cmap=CMAP_IGRAM, rasterized=True, vmin=0, vmax=vmax,
                    xincrease=False)
    psd.plot(x='freq_x', y='freq_y', ax=axes[1], add_colorbar=False, cmap=CMAP_IGRAM, rasterized=True,
             xincrease=False)
    for ax in axes:
        ax.set_aspect('equal')
        ax.set_xticks([])
