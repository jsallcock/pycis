...
class TestDemodulate(unittest.TestCase):
    ...
    def test_triple_delay_pixelated_demod(self, plot=True):
        ...
        delays = inst.get_delay(wl0, igram.x, igram.y)
        ...
        decimal = 3
        assert_almost_equal(demod_data[2][0].values, np.ones(igram.shape), decimal=decimal, err_msg="Demod of zeta_2 not working!")
        assert_almost_equal(demod_data[2][1].values, np.ones(igram.shape), decimal=decimal, err_msg="Demod of zeta_sum not working!")
        assert_almost_equal(demod_data[2][2].values, np.ones(igram.shape), decimal=decimal, err_msg="Demod of zeta_diff not working!")
